...
from analysis_utils import analyze_with_llm
...
def home_page():
    ...
    if len(df) > 0:
        ...
        df_chart = df[df.index >= one_year_ago]
    else:
        df_chart = df
        
    st.line_chart(df_chart[["Close", "SMA_50", "SMA_200"]].dropna())
    ...
    if uploaded_file:
        ...
        result = analyze_with_llm(template, content)
        st.markdown(result)
